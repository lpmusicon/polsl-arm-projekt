...
public:
	...
	uint32_t CompensateHumidity(int32_t adc_hum, int32_t t_fine)
	{
		...
		v_x1_u32r = ((((adc_hum << 14) - ((int32_t)m_calibrationData.dig_H4 << 20) - ((int32_t)m_calibrationData.dig_H5 * v_x1_u32r)) + (int32_t)16384) >> 15) * (((((((v_x1_u32r * (int32_t)m_calibrationData.dig_H6) >> 10) * (((v_x1_u32r * (int32_t)m_calibrationData.dig_H3) >> 11) + (int32_t)32768)) >> 10) + (int32_t)2097152) * (int32_t)m_calibrationData.dig_H2 + 8192) >> 14);
		v_x1_u32r = v_x1_u32r - (((((v_x1_u32r >> 15) * (v_x1_u32r >> 15)) >> 7) * (int32_t)m_calibrationData.dig_H1) >> 4);
...
private:
	...
	bool ReadCalibrationData()
	{
		uint16_t h4, h5;
		if (
			ReadData(BMP280_REG_HUM_CALIB, reinterpret_cast<uint8_t *>(&m_calibrationData.dig_T1), 2) && 
			ReadData(0x8a,  reinterpret_cast<uint8_t *>(&m_calibrationData.dig_T2), 2) && 
			ReadData(0x8c,  reinterpret_cast<uint8_t *>(&m_calibrationData.dig_T3), 2) && 
			ReadData(0x8e,  reinterpret_cast<uint8_t *>(&m_calibrationData.dig_P1), 2) && 
			ReadData(0x90,  reinterpret_cast<uint8_t *>(&m_calibrationData.dig_P2), 2) && 
			ReadData(0x92,  reinterpret_cast<uint8_t *>(&m_calibrationData.dig_P3), 2) && 
			ReadData(0x94,  reinterpret_cast<uint8_t *>(&m_calibrationData.dig_P4), 2) && 
			...
			ReadData(0xe4, reinterpret_cast<uint8_t *>(&h4), 2) && 
			ReadData(0xe5, reinterpret_cast<uint8_t *>(&h5), 2) && 
			ReadData(0xe7, reinterpret_cast<uint8_t *>(&m_calibrationData.dig_H6), 1))
		{
